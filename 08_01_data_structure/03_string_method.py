...
# .replace(old, new[,count]) >>> count는 앞서 old를 new로 몇개까지 바꿀것인가
...
# .strip() 은 공백(띄어쓰기, 탭, 엔터) 제거 >>> 데이터 정리에 유용
...
# split
# .split(sep=None, maxsplit=-1)  maxsplit은 최대 maxsplit번의 분할을 하겠다고 지정.
# 1. 공백을 기준으로 분리 (기본 동작)
...
# join
# 'sep'.join(iterable) >>> 반복가능한 객체를 sep로 연결.
